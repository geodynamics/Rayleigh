...
To read a user generated initial temperature field:

::

   &initial_conditions_namelist
   init_type=8
   T_init_file = '<filename>'
/

where <filename> is the name of the file generated by the python
script.

Generating the file:
For example, if a user wanted to specify a single mode in that input file then they could just run the script:
