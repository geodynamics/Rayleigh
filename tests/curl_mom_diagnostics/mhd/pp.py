...
def force_components(pind, tind, rind):
    ...
    # Inertia
    FI_r = F(1201, pind, tind, rind)
    FI_t = F(1202, pind, tind, rind)
    FI_p = F(1203, pind, tind, rind)

    # Lorentz
    FM_r = -F(1248, pind, tind, rind)
    FM_t = -F(1249, pind, tind, rind)
    FM_p = -F(1250, pind, tind, rind)

    # Coriolis
    FC_r = -F(1219, pind, tind, rind)
    FC_t = -F(1220, pind, tind, rind)
    FC_p = -F(1221, pind, tind, rind)

    # Buoyancy
    FA_r = -F(1216, pind, tind, rind)

    # Pressure
    FP_r = -F(1237, pind, tind, rind)
    FP_t = -F(1238, pind, tind, rind)
    FP_p = -F(1239, pind, tind, rind)

    # Viscous
    FV_r = -F(1228, pind, tind, rind)
    FV_t = -F(1229, pind, tind, rind)
    FV_p = -F(1230, pind, tind, rind)

    balance_r = FI_r + FC_r + FP_r + FA_r + FM_r + FV_r
    balance_t = FI_t + FC_t + FP_t + FM_t + FV_t
    balance_p = FI_p + FC_p + FP_p + FM_p + FV_p
