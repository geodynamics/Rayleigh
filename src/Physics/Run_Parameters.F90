...
Module Run_Parameters
   ...
   !     variable_name = "---short_name---@@short_name@@---"//Char(0)
   ! --The @@short_name@@ instructs the python routine where to insert the variable value
   ! --The rest allows the following shell command to extract the value of the variable
   ! from the binary executable file using something like (shown for bash shell):
   !     GitURL=`strings rayleigh.opt | grep -m1 -o -P "(?<=\-\-\-git url\-\-\-).*(?=\-\-\-)"`
   !     echo "Git URL = "${GitURL}
   Character(len=128) :: build_git_commit = "---git hash---@@git hash@@---"//Char(0)
   Character(len=128) :: build_git_url = "---git url---@@git url@@---"//Char(0)
   Character(len=128) :: build_git_branch = "---git branch---@@git branch@@---"//Char(0)
   Character(len=128) :: build_date = "---build date---@@date@@---"//Char(0)
   Character(len=128) :: build_machine = "---build machine---@@machine@@---"//Char(0)
   Character(len=512) :: build_dir = "---build dir---@@directory@@---"//Char(0)
   Character(len=512) :: build_custom_dir = "---custom dir---@@custom dir@@---"//Char(0)
   Character(len=64 ) :: build_version = "---build version---@@build version@@---"//Char(0)
   Character(len=512) :: build_fflags = "---build fflags---@@fflags@@---"//Char(0)
   Character(len=512) :: build_lib = "---build lib---@@library@@---"//Char(0)
   Character(len=512) :: FC_version = "---FC version---@@FC version@@---"//Char(0)
   Character(len=512) :: FC_location = "---FC location---@@FC location@@---"//Char(0)
      ...
      If (my_rank .eq. 0) Then
          ...
          If (ierr .eq. 0) Then
             ...
             Write(io,1001) "Compiler Location = ", &
                             Trim(FC_location(18:Len_Trim(FC_location)-4))
             Write(io,1001) "Compiler Version  = ", &
                             Trim(FC_version(17:Len_Trim(FC_version)-4))
             Write(io,*)
             Write(io,1001) "Compiler Flags    = ", &
                             Trim(build_fflags(19:Len_Trim(build_fflags)-4))
             Write(io,1001) "Compiler Library  = ", &
                             Trim(build_lib(16:Len_Trim(build_lib)-4))
             Write(io,*)
             Write(io,999)
             Write(io,*) "Build Information"
             Write(io,999)
             Write(io,1001) "Build Date       = ", &
                             Trim(build_date(17:Len_trim(build_date)-4))
             Write(io,1001) "Build Machine    = ", &
                             Trim(build_machine(20:Len_trim(build_machine)-4))
             Write(io,1001) "Build Directory  = ", &
                             Trim(build_dir(16:Len_trim(build_dir)-4))
             Write(io,1001) "Custom Directory = ", &
                             Trim(build_custom_dir(17:Len_trim(build_custom_dir)-4))
             Write(io,*)
             Write(io,1001) "Build Version = ", &
                             Trim(build_version(20:Len_trim(build_version)-4))
             Write(io,1001) "Git Hash = ", &
                             Trim(build_git_commit(15:Len_trim(build_git_commit)-4))
             Write(io,1001) "Git URL = ", &
                             Trim(build_git_url(14:Len_trim(build_git_url)-4))
             Write(io,1001) "Git Branch = ", &
                             Trim(build_git_branch(17:Len_trim(build_git_branch)-4))
             Write(io,*)
