...
contains
  ...
  subroutine read_input(filename, field_ind, field)
    ...
    integer :: l_endian_tag, version, mode, n_lmn, l_l_max, l_n_max
    integer :: i, j, k, l, m, n, lm, mp, j1, jc, jf, jn, p, np, col_mp_min
    ...
    integer, allocatable, dimension(:) :: proc_lmn_count, col_lmn_count, col_lmn_ind1, sendarri
    ...
    if (my_rank .eq. 0) then
      ...
      read(15)mode           ! mode - tells us whether this file uses sparse (0) or dense (1) storage
      pars(1) = mode
      if (mode == 0) then
        ...
        n_lmn = 0
        write(6,*)'Unknown generic input file mode: ', mode
    ...
    if (mode == 0) then
      ...
      if (my_column_rank .eq. 0) then
        ...
        np = pfi%ccomm%np         ! number of processes in this column
        allocate(proc_lmn_count(0:np-1))
          ...
          if (j1 .gt. 0) then
            ...
            do j = j1, jf
              if ((ls(j) .eq. l) .and. (ns(j) .le. n_r)) then
                ...
                proc_lmn_count(lm_owner(k)) = proc_lmn_count(lm_owner(k)) + 1  ! increment count for this process
                lmn_inds(jn,1) = k                    ! record lm index of coeff
                lmn_inds(jn,2) = ns(j)                ! record n index of coeff
                lmn_coeffs(jn,1) = col_coeffs(j,1)    ! record coeffs
                lmn_coeffs(jn,2) = col_coeffs(j,2)
                jn = jn + 1
              end if
            end do
          end if
      ...
      if (allocated(field%p1b)) then
        ...
        fcount(:,:) = 1
        call field%init(field_count = fcount, config = 'p1b')
        call field%construct('p1b')
