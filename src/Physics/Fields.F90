...
Contains
    ...
    Subroutine Initialize_Field_Structure()
        ...
        if (.not. magnetism) then
            wsfcount(1,2) = n_equations 
            wsfcount(2,2) = n_equations 
            wsfcount(3,2) = n_equations 
        else
            emfr     = avar
            emftheta = cvar
            emfphi   = avar+1
            ! seven RHS's (plus scalars) go back for the solve (1 field is differentiated and combined at the end)
            wsfcount(1,2) = n_equations + 1  
            wsfcount(2,2) = n_equations + 1 
            wsfcount(3,2) = n_equations + 1  
        endif
