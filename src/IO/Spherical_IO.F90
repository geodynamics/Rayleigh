...
Contains
   ...
   End Subroutine Initialize_Spherical_IO
	...
	Subroutine Write_Shell_Slices_Cache(this_iter,simtime)
        ...
        if ((Shell_Slices_Buffer%output_rank) .and. (orank .eq. 0) ) responsible = 1
        ...
        If (Shell_Slices_Buffer%output_rank ) Call Shell_Slices%OpenFile_Par(this_iter, error)
        ...
        If ( (responsible .eq. 1) .and. (shell_slices%file_open) ) Then   
            ...
            Call Shell_Slices_Buffer%write_data(filename=data_file, disp=hdisp, &
                                                file_unit = funit)
